...
EC_T_DWORD dwUseGmiiToRgmiiConv = 0;
    ...
    if (!ParseDword(ptcWord, lpCmdLine, tcStorage, &dwUseGmiiToRgmiiConv)
...
)
{
    ...
    if (pLinkParmsAdapter->linkParms.dwInstance <= 1 || pLinkParmsAdapter->linkParms.dwInstance > 2)
    {
    	OsDbgMsg("Port number must be between 1 and 2\n");
        dwRetVal = EC_E_INVALIDPARM;
        goto Exit;
    }
    ...
    EC_T_LINK_PARMS** ppLinkParms)
{
    ...
    else
    {
        ...
        { 
            ...
            {
                OsDbgMsg("Instance number must be 2\n");
                dwRetVal = EC_E_INVALIDPARM;
                goto Exit;
            }
        }
        ...
        else if (0 == OsStricmp((*ptcWord), "custom"))
        {
            ...
            {
                OsDbgMsg("Invalid eDW3504Type value\n");
                dwRetVal = EC_E_INVALIDPARM;
                goto Exit;
            }
            /* get PHY interface */
...
EC_T_VOID ShowLinkLayerSyntax2(EC_T_VOID)
{
    ...
    OsDbgMsg("     Mode            Interrupt (0) or Polling (1)\n");
    OsDbgMsg("     RefBoard:       socrates | lces1 | rd55up06 | r12ccpu | custom\n");
    OsDbgMsg("     SoCrates board: Instance=2, PhyAddress=1\n");
    OsDbgMsg("     if custom        DW3504Type: cycloneV | lces1 \n");
    ...
    OsDbgMsg("     UseGmiiToRgmii    Use Xilinx GmiiToRgmii converter TRUE (1) or FALSE (0)\n");
    OsDbgMsg("     GmiiToRgmiiPort   GmiiToRgmii converter PHY address 0 .. 31\n");
#endif
#if (defined INCLUDE_I8254X)
...
#endif
#if (defined INCLUDE_ALTERATSE)
    OsDbgMsg("   -alteratse        Link layer = Lenze/Intel FPGA TSE\n");
    OsDbgMsg("     Instance        Port Instance (1=first), ex. 1\n");
    OsDbgMsg("     Mode            Interrupt (0) or Polling (1) mode\n");
#endif
#if (defined INCLUDE_L9218I)
