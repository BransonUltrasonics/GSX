...
if("${CMAKE_HOST_SYSTEM_NAME}" MATCHES "Windows" AND "${WIND_CMAKE_MAKE_PROGRAM}" MATCHES "ninja")
  ...
  set(CMAKE_C_LINK_EXECUTABLE
	"${PARTIAL_LINK_TOOL} ${PARTIAL_LINK_FLAGS_DKM} -o <TARGET>.partial.o <OBJECTS> && sh -c \" ${WIND_NM} ${WIND_NM_FLAGS} <TARGET>.partial.o | tclsh ${WIND_BASE}/host/resource/hutils/tcl/munch.tcl -c arm -tags C:/ScriptGSX2-3/GSX2-3Stream/AM4377_VSB/krnl/tags/dkm.tags > <TARGET>.ctdt.c \" && ${CMAKE_C_COMPILER} ${LINK_FLAGS_DKM_${VXWORKS_BUILD_TYPE}} ${CC_ARCH_SPEC} -fdollars-in-identifiers -Wall  ${ADDED_CFLAGS} ${ADDED_INCLUDES} ${DEFINES} -o <TARGET>.ctdt.o -c <TARGET>.ctdt.c && ${WIND_HOME}/compilers/gnu-4.8.1.9/x86-win32/bin/ccarm ${CC_ARCH_SPEC} ${LINK_FLAGS_DKM_TOOL} -o <TARGET> <TARGET>.partial.o <TARGET>.ctdt.o <LINK_LIBRARIES> ${LIBPATH} ${LIBS} ${ADDED_LIBPATH} ${ADDED_LIBS}")
else()
  set(CMAKE_C_LINK_EXECUTABLE
	"sh -c '${PARTIAL_LINK_TOOL} ${PARTIAL_LINK_FLAGS_DKM} -o <TARGET>.partial.o <OBJECTS> && ${WIND_NM} ${WIND_NM_FLAGS} <TARGET>.partial.o | tclsh ${WIND_BASE}/host/resource/hutils/tcl/munch.tcl -c arm -tags C:/ScriptGSX2-3/GSX2-3Stream/AM4377_VSB/krnl/tags/dkm.tags > <TARGET>.ctdt.c && ${CMAKE_C_COMPILER} ${LINK_FLAGS_DKM_${VXWORKS_BUILD_TYPE}} ${CC_ARCH_SPEC} -fdollars-in-identifiers -Wall  ${ADDED_CFLAGS} ${ADDED_INCLUDES} ${DEFINES} -o <TARGET>.ctdt.o -c <TARGET>.ctdt.c && ${WIND_HOME}/compilers/gnu-4.8.1.9/x86-win32/bin/ccarm ${CC_ARCH_SPEC} ${LINK_FLAGS_DKM_TOOL} -o <TARGET> <TARGET>.partial.o <TARGET>.ctdt.o <LINK_LIBRARIES> ${LIBPATH} ${LIBS} ${ADDED_LIBPATH} ${ADDED_LIBS}'")
endif()
